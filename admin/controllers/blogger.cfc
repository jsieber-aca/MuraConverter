<cfcomponent extends="controller" output="false">
	...
	<cffunction name="import" output="false" returntype="any">
		...
		<cfset var importDirectory = expandPath(rc.$.siteConfig('assetPath')) & '/assets/file/muraConverter/bloggerImport/' />
		<cfset var rawXML = "" />
		<cfset var blgrXML = "" />
		<cfset var item = "" />
		...
		<cffile action="upload" filefield="bloggerXML" destination="#importDirectory#" nameConflict="makeunique" result="uploadedFile">
		...
		<cfset blgrXML = xmlParse(rawXML) />
		<cfdump var="#blgrXML#" top="3" abort="true">
		<cfloop condition="allParentsFound eq false">
			<cfset allParentsFound = true />
			<cfloop array="#blgrXML.rss.channel.item#" index="item">
				<cfscript>
